...
def get_NaHCO3_sol_vector_as_dict(x):
    logc = {}
...
def NaHCO3_equilibrium(x, cNaHCO3, TK, pCO2):
    logc = get_NaHCO3_sol_vector_as_dict(x)
    cNaT = cNaHCO3 #This came from sodium carbonate
    # ionic strength
    I = calc_ionic_strength_NaHCO3(logc)
    ion_activity(I,TK)
...
def calc_ionic_strength_NaHCO3(logc):
...
def get_CaCl2_sol_vector_as_dict(x):
    logc = {}
    logc['H+'] = x[0]; logc['OH-'] = x[1]; logc['CO2'] = x[2]; logc['CO3--'] = x[3]; \
...
def CaCl2_equilibrium(x,cCaCl2, TK, pCO2):
    logc = get_CaCl2_sol_vector_as_dict(x)
...
def get_CaCO3mixture_sol_vector_as_dict(x):
    logc = {}
    logc['H+'] = x[0]; logc['OH-'] = x[1]; logc['CO2'] = x[2]; logc['CO3--'] = x[3]; \
...
def CaCO3_mixture(x,cCaT,DIC,cNaT,cClT, TK):
    logc = get_CaCO3mixture_sol_vector_as_dict(x)
    # Calculate the ionic strength
    I = calc_ionic_strength_CaCO3_mixture(logc)
    ion_activity(I, TK)
...
def calc_ionic_strength_CaCO3_mixture(logc):
...
def create_mix_guess_from_NaHCO3_equilibrium(x_nahco3):
    # x_guess = np.full(16, -23.41132)
    x_guess = np.full(16, -1e-2)
    x_guess[0:5] = x_nahco3[0:5]
    x_guess[9:13] = x_nahco3[5:9]

    return x_guess

def calc_NaHCO3_equilibrium_open(cNaHCO3, TK, pCO2 = 0.00035075):
    solNaHCO3 = solve_with_exception(NaHCO3_equilibrium, np.full(10, -1.0), (cNaHCO3, TK, pCO2))
    logc = get_NaHCO3_sol_vector_as_dict(solNaHCO3.x)
    pHNaHCO3 = -logc['H+']-loggamma['H+']
    DICNaHCO3 = 10**logc['CO2']+10**logc['CO3--'] + 10**logc['HCO3-'] + 10**logc['NaCO3-'] \
        + 10**logc['NaHCO3'] + 10**logc['Na2CO3']
    I = calc_ionic_strength_NaHCO3(logc)
    return pHNaHCO3, DICNaHCO3, I, solNaHCO3.x

def solve_with_exception(fun, x0, args):
    sol = optimize.root(fun, x0, args=args, method='hybr')
    if sol.success != True:
...
def equilibrium_open(cCT, cCaT, cNaT, cClT, TK, pCO2): #(cNaHCO3, cCaCl2, QCaCl2, QNaHCO3, T):
# def equilibrium(cNaHCO3, cCaCl2, QCaCl2, QNaHCO3, T):

    ########## NaHCO3 equilibrium  ##########
    cNaHCO3 = cNaT
    try:
        pHNaHCO3, DICNaHCO3, INaHCO3, x_sol_nahco3 = calc_NaHCO3_equilibrium_open(cNaHCO3, TK, pCO2)
    except ValueError:
        DICNaHCO3 = 0
        pHNaHCO3 = -1
    ...
    cCaCl2 = cCaT
    try:
        solCaCl2 = solve_with_exception(CaCl2_equilibrium, np.full(11, -1.0), args=(cCaCl2, TK, pCO2))
        logc = get_CaCl2_sol_vector_as_dict(solCaCl2.x)
        pHCaCl2 = -logc['H+']-loggamma['H+']
        DICCaCl2 = 10**logc['CO2'] + 10**logc['CO3--'] + 10**logc['HCO3-'] + 10**logc['CaHCO3+'] + 10**logc['CaCO3']
    except ValueError:
        DICCaCl2 = 0
        pHCaCl2 = -1

    DIC = DICNaHCO3 + DICCaCl2
    x_guess = create_mix_guess_from_NaHCO3_equilibrium(x_sol_nahco3)
    solmix = optimize.root(CaCO3_mixture, x_guess, args=(cCaT,DIC,cNaT,cClT, TK), method='hybr')
    if solmix.success != True:
        print('Error in numerical method during mixing solution!!')

    logc = get_CaCO3mixture_sol_vector_as_dict(solmix.x)
    pHmix = -logc['H+']-loggamma['H+']
    DICmix = 10**logc['CO2'] + 10**logc['CO3--'] + 10**logc['HCO3-'] + 10**logc['CaHCO3+'] + 10**logc['CaCO3']

    logK = calculate_log10_equilibrium_constant(TK)
    IAP = np.power(10,logc['Ca++']+loggamma['Ca++'])*np.power(10,logc['CO3--']+loggamma['CO3--'])
    S = IAP/np.power(10.0,logK['calcite'])
    IS = np.log10(S)



    results = [pHNaHCO3, pHCaCl2, IS, pHmix, IAP, solmix]
    return results

def equilibrium_closed(cCT, cCaT, cNaT, cClT, TK, x_guess = np.array([])):
    if len(x_guess) == 0:
        pCO2 = 0.00035075
        solNaHCO3 = solve_with_exception(NaHCO3_equilibrium, np.full(10, -1.0), (cNaT, TK, pCO2))
        x_guess = create_mix_guess_from_NaHCO3_equilibrium(solNaHCO3.x)
    else:
        x_guess = np.full(16, -1e-2)
    solmix = optimize.root(CaCO3_mixture, x_guess, args=(cCaT,cCT,cNaT,cClT, TK), method='hybr')
    if solmix.success != True:
        print('Error in numerical method during mixing solution!!')
    logc = get_CaCO3mixture_sol_vector_as_dict(solmix.x)
    pHmix = -logc['H+']-loggamma['H+']
    DICmix = 10**logc['CO2'] + 10**logc['CO3--'] + 10**logc['HCO3-'] + 10**logc['CaHCO3+'] + 10**logc['CaCO3']
    IAPcalc = np.power(10,logc['Ca++']+loggamma['Ca++'])*np.power(10,logc['CO3--']+loggamma['CO3--'])
    I = calc_ionic_strength_CaCO3_mixture(logc)

    return solmix.x, pHmix, I, -1.0, -1.0, IAPcalc, I, -1.0, -1.0, DICmix
...
def test_solve_from_dic(cNaHCO3, cCaCl2):
    x_guess = [] #not used
    cCaT = cCaCl2
    cCT = cNaHCO3
    cClT = 2*cCaCl2
    ...
    res = equilibrium_closed(cCT, cCaT, cNaT, cClT, TK, x_guess)
    print('NaHCO3 open system pH={}; DIC={}, I={}'.format(pHNaHCO3, DICNaHCO3, INaHCO3))
    print('NaHCO3 closed system pH={}; DIC={}, I={}'.format(res[1], res[-1], res[2]))
    # print('pH= ', res[1])
    # print('I= ', res[2])
    return
...
# OUTPUT PROGRAM
###############################################################
def calculate_properties(cNaHCO3, cCaCl2, QCaCl2, QNaHCO3, T, pCO2):
    cCaT = cCaCl2
    cCT = cNaHCO3
    cClT = 2*cCaCl2
    cNaT = cNaHCO3
    ...
    results = equilibrium_open(cCT, cCaT, cNaT, cClT, TK, pCO2)
    IS = results[2]
...
if __name__== "__main__":
    ...
    ################ simple solve ##############################
    test_solve_from_dic(cNaHCO3, cCaCl2)

    ################ Calculating ##############################
    results, fig = calculate_properties(cNaHCO3, cCaCl2, QCaCl2, QNaHCO3, T, pCO2)
    [pHNaHCO3, pHCaCl2, IS, pH, IAP, solmix] = results
    ################ Output ##############################
